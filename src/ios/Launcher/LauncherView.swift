...
struct LauncherView: View {
  ...
  @State private var savedConfigSortMode: SavedConfigSortMode = .name
  ...
  var body: some View {
    VStack {
      HStack {
        ZStack {
          ...
          HStack {
            ...
            Spacer()
            if launcherMode == .savedConfigs {
              Button {
                if savedConfigSortMode == .name {
                  savedConfigSortMode = .lastRanAt
                } else {
                  savedConfigSortMode = .name
                }
              } label: {
                switch savedConfigSortMode {
                case .name:
      ...
      if launcherMode == .launcher {
        ...
        LauncherConfigsView(viewModel: viewModel, launcherMode: $launcherMode, showToast: $showToast, sortMode: $savedConfigSortMode)
      }
      ColoredText("Ported to ^[iOS](colored: 'green') by ^[Yoshi Sugawara](colored: 'purple'), 2023. Launcher brought to you by ^[SwiftUI](colored: 'indigo')").font(.small).foregroundColor(.gray)
